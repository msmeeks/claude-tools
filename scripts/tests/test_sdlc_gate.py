...
def test_gate_persists_head_as_baseline_and_clears_bookkeeping_on_completion(tmp_path):
    ...
    assert loaded["sdlc_finding_issues"] == []
...
def test_run_sdlc_review_gate_resumes_triage_after_waiting_out_a_limit(tmp_path):
    ...
    # Per-round bookkeeping is cleared once the round completes (round-scoped, see
    # test_gate_persists_head_as_baseline_and_clears_bookkeeping_on_completion).
    assert data["sdlc_finding_issues"] == []

...
- **`pending`** (default, and the only non-terminal value): runs `run_sdlc_review_gate`, which works through three phases inside a retry loop (`attempt` 1…`MAX_REVIEW_ATTEMPTS`):
  ...
  3. **Triage** — for every filed issue, runs the real `/triage` skill non-interactively — self-answering any grilling-style open questions with a stated recommendation and rationale instead of pausing for user input — then applies `/triage`'s normal state machine. Every issue that reaches `ready-for-agent` is clustered (à la `/plan-iteration` Step 5) into new `meta/plans/<slug>.md` plan files appended to `prd.json`, without opening a new integration branch or PR.

  The triage step logs one line per issue to `meta/plans/implementation-logs/run-next-plan-*-triage.log` and finishes by printing `TRIAGE_DONE`.
